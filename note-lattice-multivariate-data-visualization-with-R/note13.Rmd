...
panel.bwtufte <- function(x, y, coef = 1.5, ...) {
  ...
  panel.segments(
    x0 = rep(ux, 2),
    y0 = c(blist.stats[, 1], blist.stats[, 5]), x1 = rep(ux, 2),
...
panel.corrgram <-
  function(x, y, z, subscripts, at,
    ...
    if (label) {
      panel.text(
        x = x, y = y, lab = 100 * round(z, 2), cex = 0.8,
...
xyplot(y ~ x, county.map,
  ...
  key =
    list(
      text = list(levels(modal.ancestry), adj = 1),
...
## Figure 13.9
cloud(zz ~ xx * yy, county.map,
  par.box = list(col = "grey"),
